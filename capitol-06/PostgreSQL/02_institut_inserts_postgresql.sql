...
--
--  Aquest fitxer omple les taules creades per institut_postgresql.sql
...
--    · Filtres amb HAVING, IN, BETWEEN, LIKE / ILIKE, IS NULL...
--    · Funcions de data, text i nombre, EXISTS / NOT EXISTS, CTE...
-- ============================================================

\c institut

-- ------------------------------------------------------------
--  Neteja prèvia per poder reexecutar l'script.
--  RESTART IDENTITY reinicia les seqüències de les columnes IDENTITY,
--  CASCADE força el TRUNCATE encara que hi hagi dependències FK.
-- ------------------------------------------------------------
TRUNCATE TABLE matricula, modul, alumne, professor, departament
    RESTART IDENTITY CASCADE;

-- ------------------------------------------------------------
--  DEPARTAMENTS
--  Les columnes id_dept estan declarades com a GENERATED ALWAYS AS IDENTITY,
--  per això cal OVERRIDING SYSTEM VALUE si volem fixar l'id manualment.
-- ------------------------------------------------------------
INSERT INTO departament (id_dept, nom_dept, edifici) OVERRIDING SYSTEM VALUE VALUES
...
-- Sincronitzem la seqüència IDENTITY amb l'últim id inserit
SELECT setval(pg_get_serial_sequence('departament','id_dept'),
              (SELECT MAX(id_dept) FROM departament));

-- ------------------------------------------------------------
--  PROFESSORS
-- ------------------------------------------------------------
INSERT INTO professor (id_prof, nom, cognoms, email, telefon, salari, data_alta, actiu, id_dept)
...
SELECT setval(pg_get_serial_sequence('professor','id_prof'),
              (SELECT MAX(id_prof) FROM professor));

-- ------------------------------------------------------------
--  ALUMNES
-- ------------------------------------------------------------
INSERT INTO alumne (id_alumne, nom, cognoms, email, data_naix, telefon, curs_inici)
OVERRIDING SYSTEM VALUE VALUES
...
SELECT setval(pg_get_serial_sequence('alumne','id_alumne'),
              (SELECT MAX(id_alumne) FROM alumne));

-- ------------------------------------------------------------
--  MÒDULS
-- ------------------------------------------------------------
--  ADM201 (Gestió logística) es deixa amb id_prof = NULL per practicar LEFT JOIN.
INSERT INTO modul (id_modul, codi, nom_modul, hores, curs, id_prof)
OVERRIDING SYSTEM VALUE VALUES
    -- Informàtica
...
SELECT setval(pg_get_serial_sequence('modul','id_modul'),
              (SELECT MAX(id_modul) FROM modul));
...
INSERT INTO matricula (id_alumne, id_modul, nota_final, convocatoria) VALUES
    ...
    -- Pol (3) — Bases de dades amb 3 convocatòries
    ( 3,  1, 4.25, 1),
    ( 3,  1, 3.00, 2),
    ( 3,  1, 5.10, 3),
    ( 3,  2, 5.00, 1),
    ...
    -- Júlia (4) — Disseny curs 2
    ( 4, 10, 8.75, 1),
    ( 4,  8, 7.00, 1),
    ( 4, 12, 5.50, 1),

    -- Aleix (5) — Informàtica curs 1
    ( 5,  1, NULL, 1),
    ( 5,  2, 7.80, 1),
    ( 5,  3, 9.10, 1),
    ...
    -- Roger (12) — Informàtica curs 2 amb arrossegades
    (12,  6, 4.00, 1),
    (12,  6, 5.20, 2),
    (12,  7, 6.00, 1),
